...
if types.ntype == 1
    USpout(end-1, :) =-2*mu*deltaX/deltaY;
    USuout(end-1, :) = 2*mu*bounds.u.n*deltaX/deltaY;
end
if types.stype == 1
    USpout(2, :) =-2*mu*deltaY/deltaY;
    USuout(2, :) = 2*mu*bounds.u.s*deltaY/deltaY;
end


if types.wtype == 1
    VSpout(:, 2) =-2*mu*deltaX/deltaX;
    VSuout(:, 2) = 2*mu*bounds.v.w*deltaX/deltaX;
end
if types.etype == 1
    VSpout(:, end-1) =-2*mu*deltaY/deltaX;
    VSuout(:, end-1) = 2*mu*bounds.v.e*deltaY/deltaX;
end
